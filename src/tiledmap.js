...
//      this.map = new cc.TMXTiledMap(res.map00_tmx);
  switch(stage){
    case 1 : this.map = new cc.TMXTiledMap(res.map01_tmx);
    break;

    case 2 : this.map = new cc.TMXTiledMap(res.map02_tmx);
    break;

    case 3 : this.map = new cc.TMXTiledMap(res.map03_tmx);
    break;

    case 4 : this.map = new cc.TMXTiledMap(res.map04_tmx);
    break;

    case 5 : this.map = new cc.TMXTiledMap(res.map05_tmx);
    break;
  }
      this.init(parent);
   ...
   init: function(parent) {
      ...
      var TerrainLayer = this.map.getLayer("block");
    ...
    //  console.log("---------------------------")
      var CollisionObj = this.map.getLayer("obje");
      for (i = 0; i < mapWidth; i++) {
         for (j = 0; j < mapHeight; j++) {
            //タイルコードを取得できる
            var tileCord = new cc.Point(i, j);
            ...
            if (gid) {
              ...
              //  console.log("gid:", gid, properties["category"]);
               ...
               if (properties["blo"] ) {
                  ...
                  var objects = new Objects(parent, tileXPositon, tileYPosition,Number(properties["blo"])  );
               }
            }
         }
      }
